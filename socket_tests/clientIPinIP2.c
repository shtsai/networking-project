...
int main (int argc, char *argv[]) {
  /*
   *  This program is very similar to clientIPinIP.c, but there are some key difference.
   *  In This program, the source server is also the encapsulation server. 
   *  When source server has a packet to send to the destination server, but for some reason
   *  it cannot send the packet directly. Therefore, the source server encapsulates the packet
   *  and sends it the forwarding server. The forwarding server will decapsulate the packet,
   *  and deliver the packet to destination server.
   *
   *  This program requires four arguments:
   *     - src_server is the original server that creates this packet and encapsulates it
   *     - dst_server is the server that should receive the packet
   *     - forward_server decapsulates the IP-in-IP packet that it receives from the source and forward
  ...
  */
  ...
  /* src_server sends to forward_server, and forward_server then sends to dst_server  */ 
  struct in_addr ip1_src;
  ip1_src.s_addr = inet_addr(src_server);
